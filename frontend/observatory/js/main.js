 ...
 */
import { emptyFrame, LiveConnection } from '../wiwave-data.js';
import { WorkspaceControls } from '../wiwave-controls.js';
...
class Observatory {
  constructor() {
    ...
    // Load saved settings
    try {
      const ver = localStorage.getItem('wiwave-observatory-version');
      if (ver === SETTINGS_VERSION) {
        const saved = localStorage.getItem('wiwave-observatory-settings');
        if (saved) Object.assign(this.settings, JSON.parse(saved));
      } else {
        localStorage.removeItem('wiwave-observatory-settings');
        localStorage.setItem('wiwave-observatory-version', SETTINGS_VERSION);
      }
    } catch {}
    ...
    this._workspace = new WorkspaceControls(this);
    window.addEventListener('pagehide', () => this._disconnectWS());
  ...
  _initKeyboard() {
    window.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') { if (this._hud.settingsOpen) this._hud.toggleSettings(); this._workspace.close(); return; }
      if (this._hud.settingsOpen || this._workspace?.isOpen || /INPUT|SELECT|TEXTAREA|BUTTON/.test(e.target.tagName)) return;
      switch (e.key.toLowerCase()) {
        case 'a':
          this._autopilot = !this._autopilot;
          this._controls.enabled = !this._autopilot;
          break;
        case 'd': if (this.settings.dataSource === 'demo') this._demoData.cycleScenario(); break;
        case 'f':
  ...
  _autoDetectLive() {
    this._connection = new LiveConnection(
      frame => { this._liveData = frame; },
      state => { this._connectionState = state; }
    );
    if (!['native', 'demo', 'ws'].includes(this.settings.dataSource)) this.settings.dataSource = 'native';
    this.selectSource(this.settings.dataSource);
  }

  selectSource(mode) {
    this._disconnectWS();
    this.settings.dataSource = mode;
    this._paused = false;
    this._liveData = emptyFrame();
    this._currentData = emptyFrame();
    this._replay = null;
    this._hud._rssiHistory = [];
    this._hud._lastSparkTick = null;
    const sparkline = this._hud._sparklineCtx;
    if (sparkline) sparkline.clearRect(0, 0, sparkline.canvas.width, sparkline.canvas.height);
    this._mistPoints.geometry.attributes.alpha.array.fill(0);
    this._mistPoints.geometry.attributes.alpha.needsUpdate = true;
    this._trail.visible = false;
    if (mode === 'native') {
      this._connectWS(`${location.protocol === 'https:' ? 'wss:' : 'ws:'}//${location.host}/ws/radar`);
    } else if (mode === 'ws' && this.settings.wsUrl) {
      this._connectWS(this.settings.wsUrl);
    }
    const select = document.getElementById('opt-data-source');
    if (select) select.value = mode;
    const urlRow = document.getElementById('ws-url-row');
    if (urlRow) urlRow.style.display = mode === 'ws' ? 'flex' : 'none';
  }

  _connectWS(url) {
    try { this._connection.connect(url); }
    catch (error) { this._connectionState = error.message; this._liveData = emptyFrame('disconnected'); }
  }
  ...
  _animate() {
    ...
    // A stopped sensor never selects demonstration data.
    if (this.settings.dataSource === 'demo') {
      if (!this._paused || !this._currentData?.meta?.synthetic) this._currentData = this._demoData.update(dt);
      this._currentData.meta = { synthetic: true, fresh: true, poseAvailable: true, label: 'DEMO · SYNTHETIC', state: 'demo' };
    } else if (this.settings.dataSource === 'replay') {
      this._currentData = this._workspace.replayFrame(dt);
    } else if (!this._paused || !this._liveData?.meta?.fresh) {
      this._currentData = this._liveData || emptyFrame();
    }
    ...
    this._scenarioProps.update(data, this.settings.dataSource === 'demo' ? this._demoData.currentScenario : 'empty_room');
    this._mistPoints.visible = this.settings.dataSource === 'demo';
    this._trail.visible = this.settings.dataSource === 'demo';
    this._updateDotMatrixMist(this.settings.dataSource === 'demo' ? data : emptyFrame(), elapsed);
  ...
  _updateSignalField(data) {
    ...
    if (!field?.length) { this._fieldPoints.visible = false; return; }
    this._fieldPoints.visible = true;
...
try { new Observatory(); }
  ...
  message.textContent = 'The 3D view could not start. Enable browser hardware acceleration, or use the Live Monitor link below. ' + error.message;
  const link = document.createElement('a'); link.href = '/monitor.html'; link.textContent = 'Open Live Monitor'; message.append(link);
