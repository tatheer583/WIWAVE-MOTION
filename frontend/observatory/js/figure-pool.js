  ...
  update(data, elapsed) {
    ...
    for (let f = 0; f < this._figures.length; f++) {
      ...
      if (f < persons.length && isPresent) {
        const p = persons[f];
        const kps = p.keypoints || (data.meta?.synthetic ? this._poseSystem.generateKeypoints(p, elapsed, breathPulse) : null);
        if (!kps) { this.hide(fig); fig.visible = false; continue; }
