...
export const DEFAULTS = {
  ...
  scenario: 'auto', cycle: 30, dataSource: 'native', wsUrl: '',
};

export const SETTINGS_VERSION = 'wiwave-1';
...
export class HudController {
  ...
  initSettings() {
    ...
    dsSel.addEventListener('change', (e) => {
      ...
      obs.selectSource(e.target.value);
    ...
    document.getElementById('btn-export-settings').addEventListener('click', () => {
      ...
      a.download = 'wiwave-observatory-settings.json';
      a.click();
    });
  ...
  saveSettings() {
    try {
      localStorage.setItem('wiwave-observatory-settings', JSON.stringify(this._obs.settings));
    } catch {}
  }

  applyPreset(preset) {
    const obs = this._obs;
    const source = { dataSource: obs.settings.dataSource, wsUrl: obs.settings.wsUrl, scenario: obs.settings.scenario, cycle: obs.settings.cycle };
    Object.assign(obs.settings, preset, source);
    this.saveSettings();
    const rangeMap = {
  ...
  updateSourceBadge(dataSource, ws) {
    ...
    if (dataSource !== 'demo' && ws?.readyState === WebSocket.OPEN) {
      dot.className = 'dot dot--live'; label.textContent = 'LIVE';
    } else {
      dot.className = 'dot dot--demo'; label.textContent = dataSource === 'demo' ? 'DEMO · SYNTHETIC' : 'WAITING FOR DATA';
    }
  }
  ...
  updateHUD(data, demoData) {
    ...
    this._setText('motion-value', Number.isFinite(feat.motion_band_power) ? feat.motion_band_power.toFixed(2) : '—');

    // Mini person-count dots
    const personCount = data.estimated_persons;
    this._updatePersonDots(personCount ?? 0);
    if (personCount == null) this._setText('persons-value', '—');
    ...
    if (presEl) {
      ...
      if (cls.presence == null) { presLabel.textContent = 'UNKNOWN'; presEl.classList.add('presence--unknown'); }
    ...
    const demo = this._obs.settings.dataSource === 'demo';
    const scenarioKey = demo ? (demoData.currentScenario || 'auto') : 'live';
    if (quickSel) quickSel.disabled = !demo;
    document.getElementById('opt-scenario').disabled = !demo;
    document.getElementById('autoplay-icon').style.visibility = demo ? 'visible' : 'hidden';
  ...
  updateSparkline(data) {
    ...
    const tick = data.tick ?? data.timestamp;
    if (tick != null && tick === this._lastSparkTick) return;
    this._lastSparkTick = tick;
